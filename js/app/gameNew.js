...
let GameModule = (() => {
   ...
   return {
      ...
      insertObjIntoMap: (obj, xPos, yPos, objValue) => {
         ...
         let {width, height} = GameModule.objSizeToBlockSize(obj);
      ...
      canvasMouseHandler: () => {
         ...
         $game.mousemove((mouse) => {
            ...
            if (myMap.get(mouseTop, mouseLeft) >= 2) {
               $game.css({cursor: `url('img/game/cursors/pointing_hand.cur'), pointer`});
            } else {
               $game.css({cursor: `url('img/game/cursors/left_ptr.cur'), default`});
            }
